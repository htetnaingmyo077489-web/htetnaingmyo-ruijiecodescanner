...
def clear():
    # Windows ရော Linux/Termux မှာပါ အလုပ်လုပ်အောင် ပြင်ထားပါတယ်
...
def menu():
    ...
    # Options Section
    print(f" {G}[01]{W}[•]ENG&NUMBER (6 Digits){RE}")
    print(f" {G}[02]{W}[•]ENG LOWERCASE(6 Digits){RE}")
    print(f" {G}[03]{W}[•]NUMBER CODE (7 Digits){RE}")
    print(f" {G}[04]{W}[•]NUMBER CODE (6 Digits){RE}")
    ...
    try:
        if choice in ['1', '01']:
            ...
            print(f"\n{Y} [!] Exiting... Thanks for using Starlink!{RE}")
            sys.exit()
        else:
        ...
        # User အတွက် ပိုရှင်းတဲ့ error message
